...
import static com.colorchen.qbase.utils.Consts.GB;
import static com.colorchen.qbase.utils.Consts.MB;
...
public class DataUtil {
     ...
     */
    public static double byte2Size(long byteNum, Consts.MemoryUnit unit) {
        if (byteNum < 0) return -1;
        switch (unit) {
            default:
            case BYTE:
                return (double) byteNum / Consts.BYTE;
            case KB:
                return (double) byteNum / Consts.KB;
            case MB:
     ...
     * @return 字节数
     */
    public static long size2Byte(long size, Consts.MemoryUnit unit) {
        if (size < 0) return -1;
        switch (unit) {
            default:
            case BYTE:
                return size * Consts.BYTE;
            case KB:
                return size * Consts.KB;
            case MB:
                return size * MB;
            case GB:
                return size * GB;
    ...
    public static String byte2FitSize(long byteNum) {
        ...
        } else if (byteNum < Consts.KB) {
            return String.format(Locale.getDefault(), "%.1fB", (double) byteNum);
        } else if (byteNum < MB) {
            return String.format(Locale.getDefault(), "%.1fKB", (double) byteNum / Consts.KB);
        } else if (byteNum < GB) {
    ...
    public static ByteArrayOutputStream input2OutputStream(InputStream is) {
        ...
        try {
            ...
            byte[] b = new byte[Consts.KB];
            int len;
            while ((len = is.read(b, 0, Consts.KB)) != -1) {
                os.write(b, 0, len);
            }
            return os;
        } catch (IOException e) {
